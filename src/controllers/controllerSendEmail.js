...
const sendEmail = {
  ...
  async enviarEmailTeste(
    ...
    const texto = `<p style="font-family: sans-serif">Prezado Cliente,<br><br>Seu relatório referênte ao Orçamento: ${orcamento}, está  disponível para fazer o download.<br><br>
        Para acessar entre em nosso portal de relatórios no endereço abaixo com as seguintes informações:<br><br>
        <strong>Site: </strong>http://labsystem-nuvem.com.br/portal-cliente/<br>
        <strong>Token:</strong> ${token}<br>
        <strong>senha: </strong>  ${senha}<br><br>
        Atenciosamente <br> Laboratório Lab System.<br><br>
        Esse é um e-mail automático, favor não responder! <br> Em caso de dúvidas entre em contato com:<br><br>${table}
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        user: 'relatorioslabsystem@chat.labsystem-nuvem.com.br',
        pass: 'Relatorio@2022',
      },
    })
    ...
    try {
      await transporter.sendMail({
        ...
        // to: emails,
        // cc: emailCopia,
        subject: `Cliente: ${nomeEmpresa} Orçamento: ${orcamento}`,
        html: texto,
      })
      return true
    } catch (error) {
  ...
  async enviarEmailBoleto(orcamentos, token, senha, emails, nomeEmpresa, link) {
    if (!orcamentos || !token || !senha || !emails || !nomeEmpresa || !link) {
      console.log(orcamentos, token, senha, emails, nomeEmpresa, link)
      return false
    }

    const texto =
      '<p>Prezado Clente,<br><br>Sua nota fiscal e boletos referênte aos Orçamentos: <storng>' +
      orcamentos +
      '</storng>, estão  disponíveis para fazer o download.<br><br>' +
      'Acesse o nosso portal de relatório no endereço: http://labsystem-nuvem.com.br/portal-cliente/index.html e entre com os seguintes<br>dados abaixo: <br><br> <strong>token: </strong>' +
      token +
      '<br><br><strong>senha: </strong>' +
      senha +
      '<br><br>Atenciosamente<br><br>Laboratório Lab System.<p>'
    ...
    try {
      await transporter.sendMail({
        from: 'relatorioslabsystem@chat.labsystem-nuvem.com.br',
        to: emails,
        subject: `Cliente: ${nomeEmpresa} Orçamento: ${orcamentos}`,
        html: texto,
      })
      return true
    } catch (error) {
